...
```xml
<ItemGroup>
  <PackageReference Include="DCoding.Data.DVault.Analyzers" Version="0.22.0" PrivateAssets="all" />
</ItemGroup>
```
...
That delegation keeps the v0.22.0 generated surface small: helpers target the v1 default or read-service-compatible produced table and column shape represented by the support bundle. Projects that intentionally override produced names should stay on dynamic `IDataVaultReadService` calls or consumer-owned compiled EF queries until a later additive binding contract covers custom produced-name reads.
