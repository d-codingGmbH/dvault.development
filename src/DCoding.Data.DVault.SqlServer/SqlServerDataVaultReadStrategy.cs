...
internal sealed class SqlServerDataVaultReadStrategy :
  ...
  protected override string QuoteTableIdentifier(DbContext dbContext, string tableName) {
    ...
    var resolvedTable = ResolveTable(dbContext, tableName);
    if (resolvedTable.SchemaName is null) {
      return QuoteIdentifier(resolvedTable.TableName);
    }

    return QuoteIdentifier(resolvedTable.SchemaName) + "." + QuoteIdentifier(resolvedTable.TableName);
  }
  ...
  private int GetParentHashKeyBatchSize(DataVaultLatestSatelliteReadRequest request) {
    ...
    return Math.Min(
        ...
        SqlServerMaxCommandParameterCount - asOfParameterCount);
  }
  ...
  private readonly record struct SqlServerTableIdentifier(string TableName, string? SchemaName);
}
