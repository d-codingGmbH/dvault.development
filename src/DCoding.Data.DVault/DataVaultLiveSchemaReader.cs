...
public static class DataVaultLiveSchemaReader {
  ...
  /// <returns>
  /// A classified live-schema read result. SQLite returns a snapshot when its database is reachable; providers without a
  /// built-in reader return an unsupported-provider result instead of silently passing or throwing an unclassified failure.
  /// </returns>
  public static Task<DataVaultLiveSchemaReadResult> ReadAsync(
    ...
    return string.Equals(providerName, SqliteProviderName, StringComparison.Ordinal)
        ? SqliteReader.ReadAsync(dbContext, cancellationToken)
