...
When a provider is configured, the default report includes DVault fallback plus provider-specific optimized rows for the provider-native bulk-ingestion scenario. PostgreSQL and MySQL also include retained provider-native rows below the staged-bulk boundary so the matrix separates PostgreSQL direct/UNNEST, MySQL multi-row, and staged-provider paths without adding artifact columns. The optimized row first checks DVault diagnostics and fails the row if the named provider save strategy is not selected. The default report also keeps optional-provider read rows visible for latest satellite, PIT as-of, and bridge traversal scenarios, PostgreSQL PIT full-rebuild maintenance rows visible for the provider-neutral comparator and `PostgresDataVaultPitMaintenanceStrategy` path, and SQL Server PIT full-rebuild maintenance rows visible for the provider-neutral comparator and `SqlServerDataVaultPitMaintenanceService` path. SQLite, PostgreSQL, SQL Server, MySQL, Oracle, and DB2 latest-satellite optimized rows check read-strategy diagnostics before timing and fail if the expected provider read strategy is not selected; non-SQLite PIT and bridge optimized rows identify the provider read strategy candidate in `executionDetail`. PostgreSQL PIT maintenance rows check provider strategy selection for clean full rebuilds across the supported ordinary hub-parent, shared-driving-key multi-active hub-parent, and link-parent non-multi-active shape boundary, while SQL Server PIT maintenance rows stay bounded to the clean ordinary hub-parent full-rebuild service-replacement path. When a provider is not configured, its dependency is unavailable, or the connection cannot be opened, the command still emits provider rows with `executionStatus=skipped` and a normalized `skipReason`.

The required SQLite matrix includes read baselines for latest satellite, PIT as-of, and bridge traversal scenarios. Fixture creation, seeding, and strategy-diagnostic checks run before the timed operation so the measured read rows focus on the `IDataVaultReadService` latest satellite path, the `DataVaultPitAsOfReadRequest`/`DataVaultPitReadRecord` path, and the `DataVaultBridgeReadRequest`/`DataVaultBridgeReadRecord` path. SQLite latest-satellite, PIT, and bridge read rows compare the provider-neutral `AddDVault()` fallback with the `AddDVaultSqlite()` optimized provider read strategy, and their `executionDetail` values preserve the read-strategy status, selected strategy, fallback causes, read-shape kind, and read-shape provider status from `IDataVaultReadDiagnosticsService`. Optional PostgreSQL, SQL Server, MySQL, Oracle, and DB2 read rows remain visible in the default matrix. When those providers are skipped, the row metrics are blank/null and the planned `executionDetail` identifies the provider package path, including latest-satellite/PIT/bridge strategy candidates for the supported provider packages. PostgreSQL PIT full-rebuild maintenance rows also remain visible when PostgreSQL is skipped: the provider-neutral comparator records `selectedStrategy=<none>` with `fallbackCauses=NoProviderSpecificStrategyRegistered`, and the optimized row records `PostgresDataVaultPitMaintenanceStrategy`, `maintenanceScope=FullRebuild`, and the deterministic `pitShapeBoundary`. SQL Server PIT full-rebuild maintenance rows likewise remain visible when SQL Server is skipped: the provider-neutral comparator records `selectedStrategy=<none>`, and the optimized row records `SqlServerDataVaultPitMaintenanceService`, `maintenanceScope=FullRebuild`, and the clean ordinary hub-parent `pitShapeBoundary`. Link-parent and multi-active PIT shapes remain diagnostics and integration-test evidence unless a benchmark row explicitly names those unsupported provider-neutral fallback shapes.
...
- PostgreSQL PIT full-rebuild maintenance: ordinary hub-parent, shared-driving-key multi-active hub-parent, and link-parent non-multi-active PIT full rebuilds, measured through `IDataVaultPitMaintenanceService.RebuildAsync(...)` for the provider-neutral comparator and the `PostgresDataVaultPitMaintenanceStrategy` row
...
When PostgreSQL is configured and reachable in the default matrix, the PIT full-rebuild maintenance scenario emits:

- `dvault-adddvault-fallback` using provider-neutral PIT maintenance on the same Npgsql-backed workload
- `dvault-adddvaultpostgres-optimized` using `PostgresDataVaultPitMaintenanceStrategy` for clean full rebuilds of the supported PostgreSQL shape boundary

When SQL Server is configured and reachable in the default matrix, the PIT full-rebuild maintenance scenario emits:

- `dvault-adddvault-fallback` using provider-neutral PIT maintenance on the same SQL Server-backed workload
- `dvault-adddvaultsqlserver-optimized` using `SqlServerDataVaultPitMaintenanceService` for clean ordinary hub-parent full rebuilds
