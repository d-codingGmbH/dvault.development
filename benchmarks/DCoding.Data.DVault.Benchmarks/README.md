...
Read baselines emit the current provider-neutral read-service path through the DVault fallback registration and selected provider package rows. For SQLite reads, the provider package row uses the SQLite optimized read strategy for supported hub-parent, non-multi-active latest/as-of satellite reads, supported maintained PIT reads, and supported many-to-many or hierarchy bridge reads. SQL Server latest-satellite, PIT, and bridge rows name `SqlServerDataVaultReadStrategy` as the planned provider read strategy, but the row remains a skipped placeholder when `DVAULT_TEST_SQLSERVER_CONNECTION_STRING` is absent. Optional PostgreSQL, MySQL, Oracle, and DB2 read rows stay in the artifact triplet as skipped rows when their connection strings are absent; PIT and bridge rows name their planned provider read strategy, while latest-satellite rows retain `selectedStrategy=<none>` until a provider-specific latest-satellite read strategy is implemented and measured for that provider.
