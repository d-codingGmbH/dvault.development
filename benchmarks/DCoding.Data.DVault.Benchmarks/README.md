...
Read baselines emit the current provider-neutral read-service path through the DVault fallback registration and selected provider package rows. For SQLite reads, the provider package row uses the SQLite optimized read strategy for supported hub-parent, non-multi-active latest/as-of satellite reads, supported maintained PIT reads, and supported many-to-many or hierarchy bridge reads. Optional PostgreSQL, SQL Server, MySQL, and Oracle read rows stay in the artifact triplet as skipped rows when their connection strings are absent; PIT and bridge rows name their planned provider read strategy, while latest-satellite rows retain `selectedStrategy=<none>` until a non-SQLite latest-satellite read strategy is implemented and measured.
