...
- [ ] Treat [v0.18.0 release notes](releases/v0.18.0.md) as the current public baseline for coordinated package scope, EF safety and preflight behavior, opt-in telemetry, support-bundle export, compiled-model/query/pooling guidance, performance evidence, compatibility notes, limitations, and validation evidence.
...
- [ ] Use the root `benchmark-summary.md`, `benchmark-summary.csv`, and `benchmark-summary.json` files plus the checked-in `artifacts/benchmarks/...` bundles as performance evidence. Keep PostgreSQL, SQL Server, MySQL, and Oracle optional-provider rows visible as completed or skipped instead of silently removing unavailable external providers.
