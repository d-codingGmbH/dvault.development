...
- [ ] Do not make ordinary production adoption depend on automatic PIT or bridge maintenance orchestration, delete-aware hierarchy shrinking, link-parent PIT behavior, multi-active PIT behavior, non-SQLite PIT/bridge read optimization, provider-specific physical tuning, custom hook matrices, or unpublished provider capabilities.
...
- PIT-backed reads operate over PIT tables maintained through explicit `IDataVaultPitMaintenanceService` rebuild or parent-maintenance calls; those calls may use explicit PIT metadata or registry-backed exact logical-name/CLR-mapping PIT resolution. Reads do not refresh PIT rows implicitly. Bridge maintenance is available as an explicit caller-invoked service that rebuilds or incrementally maintains one bridge from persisted source-link rows; it is not automatic or delete-aware.
