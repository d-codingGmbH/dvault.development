...
- [ ] Treat [v0.19.0 release notes](releases/v0.19.0.md) as the current public baseline for coordinated package scope, streaming explicit-save guidance, EF safety and preflight behavior, opt-in telemetry, support-bundle export, compiled-model/query/pooling guidance, performance evidence, compatibility notes, limitations, and validation evidence.
- [ ] Treat [v0.20.0 release notes](releases/v0.20.0.md) as the provider-specific optimized write-path documentation boundary when planning that release. v0.19.0 remains the historical streaming-save baseline; v0.20.0 documents staged PostgreSQL/MySQL lanes, SQL Server native-bulk wording, Oracle retained direct optimization, and stored-procedure caveats without changing the public write API.
