...
- [ ] Do not make ordinary production adoption depend on automatic PIT or bridge maintenance orchestration, delete-aware hierarchy shrinking, link-parent PIT behavior, incompatible multi-active PIT driving-key families, cross-product tuple semantics, non-SQLite PIT/bridge read optimization, provider-specific physical tuning, custom hook matrices, or unpublished provider capabilities.
...
- PIT-backed reads operate over PIT tables maintained through explicit `IDataVaultPitMaintenanceService` rebuild or parent-maintenance calls; those calls may use explicit PIT metadata or registry-backed exact logical-name/CLR-mapping PIT resolution. Hub-parent PITs support ordinary satellites and the bounded shared-driving-key multi-active tuple baseline; link-parent PITs, incompatible driving-key families, cross-product tuple semantics, tuple-filter requests, automatic orchestration, and provider-specific PIT maintenance remain outside the current baseline. Reads do not refresh PIT rows implicitly. Bridge maintenance is available as an explicit caller-invoked service that rebuilds or incrementally maintains one bridge from persisted source-link rows; it is not automatic or delete-aware.
