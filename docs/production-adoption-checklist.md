...
- [ ] Treat [v0.20.0 release notes](releases/v0.20.0.md) as the current public baseline for coordinated package scope, streaming explicit-save guidance, staged PostgreSQL/MySQL ordered-bulk guidance, SQL Server native-bulk wording, Oracle retained direct optimization, stored-procedure caveats, EF safety and preflight behavior, opt-in telemetry, support-bundle export, compiled-model/query/pooling guidance, performance evidence, compatibility notes, limitations, and validation evidence.
...
- v0.20.0 staged bulk guidance is provider-specific, not universal. PostgreSQL and MySQL have documented staged lanes; SQL Server remains native bulk, and Oracle remains direct optimized batching until measured evidence selects staged Oracle bulk.
