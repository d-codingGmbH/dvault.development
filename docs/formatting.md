...
The same gate invokes `tools/check-one-member-per-file.sh` to enforce one top-level declaration per C# file across the repository. See `docs/quality/one-member-per-file.md` for the source-layout policy and scan scope.
