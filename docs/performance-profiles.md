...
Use [Provider Optimization Gap Matrix](plans/provider-optimization-gap-matrix.md) as the canonical follow-up recommendation surface. Its P0-P3 rows are planning backlog entries: PostgreSQL `latest-satellite-read` remains a capability-gap recommendation, SQL Server, MySQL, Oracle, and DB2 `latest-satellite-read` rows remain evidence-gap recommendations until provider-configured benchmark triplets exist, PostgreSQL, SQL Server, and MySQL `pit-as-of-read` and `bridge-traversal-read` are closed evidence rows when cited through the checked-in v0.32.0 smoke-read bundle, and PostgreSQL, SQL Server, MySQL, Oracle, and DB2 `provider-native-bulk-ingestion` plus Oracle and DB2 remaining `pit-as-of-read` and `bridge-traversal-read` rows remain evidence-gap recommendations until provider-configured benchmark triplets exist. The root quick baseline still preserves skipped PostgreSQL, SQL Server, and MySQL placeholders when their connection strings are unset. Gap-matrix recommendations do not promote skipped placeholders, diagnostics-only rows, smoke-only rows, or storage-footprint rows into completed provider timing evidence.
...
The v0.32.0 smoke read baseline is the authoritative completed-timing bundle for PostgreSQL, SQL Server, and MySQL `pit-as-of-read` and `bridge-traversal-read` rows that selected their provider read strategies. SQL Server and MySQL latest-satellite timing remains separate from this PIT/bridge closure, and skipped root quick-baseline rows remain placeholders rather than timing claims.
...
- Provider-read evidence separates completed SQLite latest-satellite, PIT, and bridge timing rows plus completed PostgreSQL, SQL Server, and MySQL PIT/bridge rows in the v0.32.0 smoke-read bundle from root optional-provider read guidance rows that may remain skipped when connection strings are unset.
...
The checked-in rows focus on `IDataVaultReadService` latest satellite reads, `DataVaultPitAsOfReadRequest`/`DataVaultPitReadRecord`, and `DataVaultBridgeReadRequest`/`DataVaultBridgeReadRecord`. SQLite is the only completed optimized latest-satellite timing row in the root benchmark triplet. PostgreSQL PIT/bridge completed timing is available from the v0.32.0 smoke-read bundle, where `PostgresDataVaultReadStrategy` was selected for supported maintained read shapes. The same bundle adds completed SQL Server PIT and bridge timing rows that selected `SqlServerDataVaultReadStrategy` and completed MySQL PIT and bridge timing rows that selected `MySqlDataVaultReadStrategy`; SQL Server and MySQL latest-satellite timing remains separate from this PIT/bridge closure. Oracle and DB2 PIT/bridge strategy candidates remain diagnostics-gated repository paths, and new live external-provider execution remains opt-in through provider connection-string configuration.
...
All values in this section are from the root quick benchmark triplet. External-provider completed rows, including PostgreSQL, SQL Server, and MySQL PIT/bridge timing, should be cited from their linked artifact bundles with preserved run context instead of copied into this quick table:
...
Provider-configured PostgreSQL, SQL Server, and MySQL PIT/bridge rows should cite [the v0.32.0 smoke-read bundle](../artifacts/benchmarks/v0.32.0-06F9XD26D2MHVAKZ2GCZ67BEFC-smoke-read-20260607/benchmark-summary.md), not the skipped root quick placeholders:
