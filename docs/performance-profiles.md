...
Status: v0.100.0 binary-default, provider parity, analyzer, privacy adoption, and migration-validation baseline with carried-forward v0.49.0 modeling/generator parity and v0.47.0 provider maintenance documentation

This guide is the detailed performance-profile reference for the current DVault performance-guidance baseline. It carries forward the v0.31.0 adopter decision tree, the v0.32.0 provider-threshold evidence bundles, the checked-in DB2 hotspot evidence bundle, the review-only provider-specific SQL artifact gate, the v0.42.0 provider evidence promotion rules and tuning gates, the v0.43.0 allocation-hotspot evidence, the source/test-backed PIT maintenance boundary for PostgreSQL, SQL Server, and MySQL, and the new v0.100.0 provider binary-default closure bundle. It does not create automatic routing, absolute performance guarantees, provider service-level objectives, dashboards, hosted observability, database provisioning, scheduler templates, credential-management guidance, automatic hash-key migration, automatic PIT or bridge maintenance, raw SQL or physical-plan promises, deployable provider-specific SQL payload generation, runtime artifact dispatch, package-publication approval, provider-specific bridge maintenance, or provider support outside the evidence-linked rows. The coordinated release record for the current package, analyzer, privacy adoption, hash-key storage migration manifest validation, provider binary-default closure, and adoption baseline is [DVault v0.100.0 Release Notes](releases/v0.100.0.md). The coordinated release record for the modeling/generator parity baseline is [DVault v0.49.0 Release Notes](releases/v0.49.0.md). The coordinated release record for the provider-maintenance documentation boundary is [DVault v0.47.0 Release Notes](releases/v0.47.0.md). Earlier release notes remain historical feature-introduction records.
...
Use the v0.100.0 provider binary-default closure bundle as the current provider-configured completed-timing source for SQLite, PostgreSQL, SQL Server, MySQL, Oracle, and DB2 rows under `sha256-v1-binary` physical hash-key storage:

- [SQLite binary-default benchmark summary](../artifacts/benchmarks/v0.100.0-provider-binary-closure-20260702/sqlite/benchmark-summary.md)
- [PostgreSQL binary-default benchmark summary](../artifacts/benchmarks/v0.100.0-provider-binary-closure-20260702/postgres/benchmark-summary.md)
- [SQL Server binary-default benchmark summary](../artifacts/benchmarks/v0.100.0-provider-binary-closure-20260702/sqlserver/benchmark-summary.md)
- [MySQL binary-default benchmark summary](../artifacts/benchmarks/v0.100.0-provider-binary-closure-20260702/mysql/benchmark-summary.md)
- [Oracle binary-default benchmark summary](../artifacts/benchmarks/v0.100.0-provider-binary-closure-20260702/oracle/benchmark-summary.md)
- [DB2 binary-default benchmark summary](../artifacts/benchmarks/v0.100.0-provider-binary-closure-20260702/db2/benchmark-summary.md)

The v0.100.0 bundle has zero failed rows in the provider subdirectories: SQLite completed 28 rows, PostgreSQL completed 8, SQL Server completed 7, MySQL completed 7, Oracle completed 5, and DB2 completed 5. Those runs are one-iteration Windows/.NET 10.0.9 timing evidence, not universal performance guarantees. PostgreSQL and DB2 were validated through the local Podman-backed test setup while preserving the same external-provider contract as the rest of the benchmark harness.

The 2026-06-23 closure bundle completed retained multi-row at `15.827` ms, bounded staged bulk at `26.055` ms, and the deliberate large mixed provider-neutral fallback at `145.601` ms. Oracle latest/PIT timings include the ODP.NET LOB-prefetch read-command tuning.

Use the 2026-06-23 provider optimization closure bundle as historical provider-configured completed-timing source for PostgreSQL, SQL Server, MySQL, Oracle, and DB2 provider-native save rows, latest-satellite read rows, and PIT/bridge read rows over already-maintained read-model rows:
...
The completed read rows in that bundle do not all have the same prerequisite. Latest-satellite rows time supported satellite reads over seeded satellite history. PIT and bridge rows time `IDataVaultReadService` reads after the PIT or bridge table has already been maintained; maintenance setup is a prerequisite for those rows, not part of the measured read timing. Cite the v0.47.0 provider-maintenance boundary below, [DVault v0.47.0 Release Notes](releases/v0.47.0.md), and [DVault V1 PIT And Bridge Boundary](architecture/dvault-v1-pit-bridge-boundary.md) for source/test-backed maintenance availability and fallback behavior, not the closure bundle as provider maintenance timing.
...
Use the v0.43 provider binary-vs-hex hash-key matrix as historical storage-profile evidence. It ran one iteration and no warmup on Windows/.NET 10.0.9 with provider filter `all`, four hash-key variants, completed PostgreSQL, MySQL, Oracle, and DB2 provider discovery, a skipped SQL Server lane for the local TLS/runtime setup, and failed binary rows for several external-provider save/read shapes. The v0.100.0 provider binary-default closure bundle supersedes those failed binary rows for the default `sha256-v1-binary` provider paths:
...
Use [Provider Optimization Gap Matrix](plans/provider-optimization-gap-matrix.md) as the canonical closure and follow-up boundary surface. Its P0-P3 rows now close PostgreSQL, SQL Server, MySQL, Oracle, and DB2 provider-native save, latest-satellite, PIT, and bridge timing with the v0.100.0 binary-default closure bundle. Remaining items in that matrix are runtime fallback boundaries and future-change reopen conditions, not open optimization gaps: skipped root placeholders, diagnostics-only rows, smoke-only rows, storage-footprint rows, dirty-context saves, unsupported read shapes, DB2 staged bulk, provider-native chunk execution, and write-side bridge maintenance push-down still require separate evidence before they can become measured provider timing claims.

## v0.47.0 Provider Maintenance Boundary

The provider PIT maintenance lanes are source and test evidence, not benchmark-backed timing evidence. The root quick triplet includes PostgreSQL `pit-full-rebuild-maintenance` skipped placeholders so row identity, fallback vocabulary, `maintenanceScope=FullRebuild`, and the supported PostgreSQL `pitShapeBoundary` stay visible when PostgreSQL is unconfigured. Do not cite the provider-maintenance work as a performance win unless a provider-configured artifact triplet preserves the exact maintenance workload, provider, run context, and fallback rows.

This section is the maintenance evidence contract for the performance guide. It complements [DVault v0.47.0 Release Notes](releases/v0.47.0.md), the historical [DVault v0.45.0 Release Notes](releases/v0.45.0.md), and [DVault V1 PIT And Bridge Boundary](architecture/dvault-v1-pit-bridge-boundary.md); it does not create a parallel maintenance timing table.

The accepted provider-maintenance baseline plus evaluated MySQL, Oracle, and DB2 expansion decisions are limited to:

- PostgreSQL: `AddDVaultPostgres()` registers `PostgresDataVaultPitMaintenanceStrategy` as an `IDataVaultProviderPitMaintenanceStrategy`. The supported optimized full-rebuild shapes are ordinary hub-parent PITs, shared-driving-key multi-active hub-parent PITs, and link-parent non-multi-active PITs when no caller transaction is already active.
...
- MySQL: `AddDVaultMySql()` registers `MySqlDataVaultPitMaintenanceStrategy` as an `IDataVaultProviderPitMaintenanceStrategy` for clean ordinary hub-parent PIT full rebuilds on the official `MySql.EntityFrameworkCore` provider only. Pomelo maintenance, shared-driving-key multi-active hub-parent PITs, and link-parent non-multi-active PITs stay provider-neutral fallback for MySQL.
- Oracle: the current decision is defer. `AddDVaultOracle()` registers Oracle save plus latest-satellite, PIT-read, and bridge-read strategies, but no Oracle PIT maintenance strategy or service replacement. Oracle completed PIT read timing is read-side evidence over already-maintained PIT rows, not rebuild proof. Any future reopen is limited to an ordinary hub-parent full-rebuild-only candidate after Oracle-specific SQL parity plus rollback-clean failure and cancellation proof; multi-active hub-parent, link-parent, and parent-maintenance shapes stay provider-neutral.
- DB2: `docs/plans/db2-pit-maintenance-full-rebuild-feasibility.md` accepts one future `IBM.EntityFrameworkCore` ordinary hub-parent `RebuildAsync(...)` lane through `IDataVaultProviderPitMaintenanceStrategy`. `AddDVaultDb2()` registers DB2 save plus latest-satellite, PIT-read, and bridge-read strategies, but no DB2 PIT maintenance strategy or service replacement today. Shared-driving-key multi-active hub-parent and link-parent full rebuilds stay deferred, while `MaintainParentsAsync(...)`, dirty contexts, provider mismatches, incomplete maintenance-shape evidence, and caller transactions without proven IBM savepoint rollback stay provider-neutral fallback.
- Fallback and rollback remain part of the claim boundary. Unsupported or mismatched maintenance requests fall back through provider-neutral maintenance. PostgreSQL caller transactions also fall back because the current provider strategy does not create a strategy-owned savepoint, MySQL caller transactions without a verified savepoint boundary fall back, and SQL Server full-rebuild faults or cancellations preserve the pre-rebuild rows.

MySQL PIT maintenance timing claims must not cite the completed MySQL `pit-as-of-read` row as maintenance proof. The implemented MySQL ordinary hub-parent maintenance lane is source/test-backed, not benchmark-backed timing evidence; it proves provider-name gating, clean-context gating, complete maintenance-shape evidence, rollback-clean delete-plus-insert execution when the strategy owns the transaction, and fallback when an ambient caller transaction cannot provide a verified savepoint boundary. Future Oracle and DB2 ordinary hub-parent implementation lanes must prove the same rollback-clean delete-plus-insert boundary before making maintenance claims.
...
| v0.100.0 provider binary-default closure bundle | Current completed timing for SQLite plus external PostgreSQL, SQL Server, MySQL, Oracle, and DB2 provider-native save rows plus latest-satellite, PIT, bridge, and supported maintenance rows under `sha256-v1-binary`. | Universal performance guarantees, provider support outside the completed rows, write-side bridge maintenance push-down, DB2 staged bulk, provider-native chunk execution, dirty-context save claims, or unsupported read shapes. |
| 2026-06-23 provider optimization closure bundle | Historical external-provider timing for PostgreSQL, SQL Server, MySQL, Oracle, and DB2 provider-native save rows plus latest-satellite, PIT, and bridge read rows. | Superseding the v0.100.0 binary-default closure bundle for current binary-default timing claims. |
...
- [v0.100.0 provider binary-default closure bundle](../artifacts/benchmarks/v0.100.0-provider-binary-closure-20260702/sqlite/benchmark-summary.md), including completed SQLite, PostgreSQL, SQL Server, MySQL, Oracle, and DB2 binary-default save/read rows across provider subdirectories
- [2026-06-23 provider optimization closure bundle](../artifacts/benchmarks/06FF0000000000000000000000-provider-optimization-closure-20260623/README.md), retained as historical PostgreSQL, SQL Server, MySQL, Oracle, and DB2 provider-native save, latest-satellite, PIT, and bridge timing context
...
The v0.100.0 provider binary-default closure bundle is the authoritative completed-timing bundle for the current closed SQLite, PostgreSQL, SQL Server, MySQL, Oracle, and DB2 provider optimization rows. The 2026-06-23 closure bundle, v0.32.0 smoke read baseline, MySQL latest-satellite ticket bundle, SQL Server bulk-threshold triplet, and DB2 hotspot bundle remain historical context and threshold rationale. Skipped root quick-baseline rows remain placeholders rather than timing claims.

The DB2 v0.100.0 closure row supersedes earlier DB2 hotspot timing rows for current guidance: `dvault-adddvaultdb2-optimized` completed at `112.416` ms under `sha256-v1-binary`, with the provider-neutral fallback comparison at `571.123` ms in the same run context.

The v0.43 provider binary-vs-hex bundle is historical evidence for hash-key variant participation. Treat completed rows as measured only with that bundle and its one-iteration run context. Treat PostgreSQL bytea/text operator failures, MySQL and DB2 truncation failures, Oracle reexecution/null-reference failures, and the skipped SQL Server local TLS lane as historical caveats superseded for the default `sha256-v1-binary` paths by the v0.100.0 provider binary-default closure bundle.
...
Treat all millisecond values below as observations from their linked run only. The v0.100.0 binary-default closure bundle records current configured provider evidence where its rows are completed, while earlier v0.32/v0.42/v0.46 bundles remain historical context. None of these rows are universal timing promises. Rerun the benchmarks when provider, hardware, runtime, load-timestamp storage, iteration count, warmup count, dataset size, request shape, or provider configuration changes.
...
- Provider-read evidence separates completed SQLite latest-satellite, PIT, and bridge timing rows, completed PostgreSQL, SQL Server, MySQL, Oracle, and DB2 read rows in the v0.100.0 binary-default closure bundle, and root optional-provider read guidance rows that may remain skipped when connection strings are unset.
...
5. Is the provider claim PIT or bridge optimization?
   ...
   SQLite, PostgreSQL, SQL Server, MySQL, Oracle, and DB2 are repository-proven diagnostics-gated PIT/bridge read-strategy candidate paths. PostgreSQL, SQL Server, MySQL, Oracle, and DB2 PIT/bridge have completed provider-configured timing in the v0.100.0 binary-default closure bundle. Other external-provider PIT/bridge timing claims need their own completed provider-configured artifact lane. Unsupported providers, provider-name mismatch, missing provider registration, strategy decline, unsupported shapes, incomplete `ReadShape` evidence, and stale read-model maintenance keep the provider-neutral read pipelines.
...
| --- | --- | --- |
| PostgreSQL | Retain direct or UNNEST below 60 operations; use staged COPY at 60-plus operations. | The v0.100.0 binary-default closure bundle completed retained direct/UNNEST at `2531.947` ms and staged COPY at `3103.227` ms, with provider-neutral fallback at `13071.782` ms. |
| SQL Server | Native bulk starts at 100-plus total operations; mixed hub/link batches start at 900-plus total operations; no more than 500 satellite operations. | The v0.100.0 binary-default closure bundle completed `SqlBulkCopy` at `189.073` ms, with provider-neutral fallback at `536.371` ms, and preserves the 100/900/500 gates. |
| MySQL | Tiny satellite-history batches fall back to provider-neutral behavior; retained multi-row starts at 50-plus operations for eligible non-large mixed batches; staged bulk is bounded to satellite-only 100-plus batches or mixed 100-to-303-operation batches. | The v0.100.0 binary-default closure bundle completed retained multi-row at `39.452` ms, bounded staged bulk at `30.605` ms, and the deliberate large mixed provider-neutral fallback row at `165.164` ms, with the provider-neutral comparator at `505.586` ms. |
| Oracle | Direct optimized batching starts at 50-plus total operations and no more than 10000 satellite operations. | The v0.100.0 binary-default closure bundle completed direct optimized save at `162.335` ms, with provider-neutral fallback at `642.158` ms, and still records `stagedOracleBulk=not-selected-no-measured-win`. |
| DB2 | Clean-context hub, link, and ordinary satellite batches can select `Db2DataVaultSaveStrategy`; no staged bulk or provider-native chunk execution is claimed. | The v0.100.0 binary-default closure bundle completed optimized clean-context save at `112.416` ms, with provider-neutral fallback at `571.123` ms. |
...
The v0.100.0 provider binary-default closure bundle is the current completed-timing evidence for PostgreSQL, SQL Server, MySQL, Oracle, and DB2 save rows. The root `benchmark-summary.csv` and `benchmark-summary.json` still keep skipped optional-provider rows visible for the quick baseline, but completed provider timing claims should cite the linked provider closure artifact that produced them.
...
- DB2: `dvault-adddvaultdb2-optimized` in the closure bundle for the completed clean-context optimized save row with `Db2DataVaultSaveStrategy` selected and `stagedBulkBoundary=not-supported`; `dvault-adddvault-fallback` in the same bundle is the provider-neutral fallback comparison row.

Current DB2 completed save rows from the v0.100.0 binary-default closure bundle:

| Provider | Scenario | Baseline | Mean ms | Evidence posture |
| --- | --- | --- | ---: | --- |
| DB2 external provider | `provider-native-bulk-ingestion` | `dvault-adddvault-fallback` | 571.123 | `completed-timing`; provider-neutral fallback comparison row with no provider-specific strategy selected and `NoProviderSpecificStrategyRegistered`. |
| DB2 external provider | `provider-native-bulk-ingestion` | `dvault-adddvaultdb2-optimized` | 112.416 | `completed-timing`; selected `Db2DataVaultSaveStrategy` for clean-context set-based save with no fallback causes. |

### Stop Conditions And Rerun Triggers

Stop before making a measured provider-specific performance claim when optional provider rows are skipped, connection strings are unset, provider packages are not restored for the benchmark run, the context has pending tracked changes, the operation count is below the provider gate, the satellite count exceeds SQL Server or Oracle limits, or diagnostics do not select the expected strategy. For DB2 save timing, cite the v0.100.0 binary-default closure bundle for the completed clean-context row and stop before claiming staged bulk, provider-native chunk execution, dirty-context saves, or unsupported save shapes. Rerun the benchmark triplet with the relevant provider configured and preserve skipped or failed rows exactly as the artifact contract requires.
...
The checked-in rows focus on `IDataVaultReadService` latest satellite reads, `DataVaultPitAsOfReadRequest`/`DataVaultPitReadRecord`, and `DataVaultBridgeReadRequest`/`DataVaultBridgeReadRecord`. SQLite is the completed optimized latest-satellite timing row in the v0.100.0 SQLite benchmark triplet. PostgreSQL, SQL Server, MySQL, Oracle, and DB2 latest-satellite/PIT/bridge completed timing is available from the v0.100.0 provider binary-default closure bundle, where each provider selected its provider read strategy for the supported latest-satellite and maintained PIT/bridge shapes.
...
All values in this quick table are from the v0.100.0 SQLite binary-default benchmark triplet. They are read timings: the PIT and bridge rows require maintained read-model rows before the benchmarked read begins, and the mean values do not include rebuild or maintenance service work. External-provider completed rows should be cited from the v0.100.0 provider binary-default closure bundle with preserved run context instead of copied into this quick table:
...
Provider-configured PostgreSQL, SQL Server, MySQL, Oracle, and DB2 read rows should cite the v0.100.0 provider binary-default closure bundle, not the skipped root quick placeholders:

| Provider | Scenario | Baseline | Mean ms | Evidence posture |
| --- | --- | --- | ---: | --- |
| PostgreSQL external provider | `latest-satellite-read` | `dvault-adddvaultpostgres-optimized` | 2519.148 | `completed-timing`; selected `PostgresDataVaultReadStrategy` with `readShape=LatestSatellite` and no fallback causes. |
| PostgreSQL external provider | `pit-as-of-read` | `dvault-adddvaultpostgres-optimized` | 2500.809 | `completed-timing`; selected `PostgresDataVaultReadStrategy` with `readShape=PitAsOf` and no fallback causes. |
| PostgreSQL external provider | `bridge-traversal-read` | `dvault-adddvaultpostgres-optimized` | 2575.714 | `completed-timing`; selected `PostgresDataVaultReadStrategy` with `readShape=Bridge` and no fallback causes. |
| SQL Server external provider | `latest-satellite-read` | `dvault-adddvaultsqlserver-optimized` | 34.848 | `completed-timing`; selected `SqlServerDataVaultReadStrategy` with `readShape=LatestSatellite` and no fallback causes. |
| SQL Server external provider | `pit-as-of-read` | `dvault-adddvaultsqlserver-optimized` | 102.708 | `completed-timing`; selected `SqlServerDataVaultReadStrategy` with `readShape=PitAsOf` and no fallback causes. |
| SQL Server external provider | `bridge-traversal-read` | `dvault-adddvaultsqlserver-optimized` | 22.649 | `completed-timing`; selected `SqlServerDataVaultReadStrategy` with `readShape=Bridge` and no fallback causes. |
| MySQL external provider | `latest-satellite-read` | `dvault-adddvaultmysql-optimized` | 17.800 | `completed-timing`; selected `MySqlDataVaultReadStrategy` with `readShape=LatestSatellite` and no fallback causes. |
| MySQL external provider | `pit-as-of-read` | `dvault-adddvaultmysql-optimized` | 30.876 | `completed-timing`; selected `MySqlDataVaultReadStrategy` with `readShape=PitAsOf` and no fallback causes. |
| MySQL external provider | `bridge-traversal-read` | `dvault-adddvaultmysql-optimized` | 10.051 | `completed-timing`; selected `MySqlDataVaultReadStrategy` with `readShape=Bridge` and no fallback causes. |
| Oracle external provider | `latest-satellite-read` | `dvault-adddvaultoracle-optimized` | 34.069 | `completed-timing`; selected `OracleDataVaultReadStrategy` with `readShape=LatestSatellite` and no fallback causes. |
| Oracle external provider | `pit-as-of-read` | `dvault-adddvaultoracle-optimized` | 64.013 | `completed-timing`; selected `OracleDataVaultReadStrategy` with `readShape=PitAsOf` and no fallback causes. |
| Oracle external provider | `bridge-traversal-read` | `dvault-adddvaultoracle-optimized` | 15.556 | `completed-timing`; selected `OracleDataVaultReadStrategy` with `readShape=Bridge` and no fallback causes. |
| DB2 external provider | `latest-satellite-read` | `dvault-adddvaultdb2-optimized` | 22.007 | `completed-timing`; selected `Db2DataVaultReadStrategy` with `readShape=LatestSatellite` and no fallback causes. |
| DB2 external provider | `pit-as-of-read` | `dvault-adddvaultdb2-optimized` | 41.071 | `completed-timing`; selected `Db2DataVaultReadStrategy` with `readShape=PitAsOf` and no fallback causes. |
| DB2 external provider | `bridge-traversal-read` | `dvault-adddvaultdb2-optimized` | 14.585 | `completed-timing`; selected `Db2DataVaultReadStrategy` with `readShape=Bridge` and no fallback causes. |
