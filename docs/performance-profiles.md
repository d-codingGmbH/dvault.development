...
The v0.32.0 smoke read baseline is the authoritative completed-timing bundle for PostgreSQL, SQL Server, MySQL, and Oracle `pit-as-of-read` and `bridge-traversal-read` rows that selected their provider read strategies. Latest-satellite timing remains separate from this PIT/bridge closure, and skipped root quick-baseline rows remain placeholders rather than timing claims.
...
- Provider-read evidence separates completed SQLite latest-satellite, PIT, and bridge timing rows plus completed PostgreSQL, SQL Server, MySQL, and Oracle PIT/bridge rows in the v0.32.0 smoke-read bundle from root optional-provider read guidance rows that may remain skipped when connection strings are unset.
...
All values in this section are from the root quick benchmark triplet. External-provider completed rows, including PostgreSQL, SQL Server, MySQL, and Oracle PIT/bridge timing, should be cited from their linked artifact bundles with preserved run context instead of copied into this quick table:
