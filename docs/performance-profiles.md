...
Status: v0.42.0 provider evidence and tuning gate documentation baseline with carried-forward v0.32.0 provider-threshold evidence, DB2 hotspot evidence, and v0.31.0 decision-tree contract

This guide is the detailed performance-profile reference for the current DVault performance-guidance baseline. It carries forward the v0.31.0 adopter decision tree, the v0.32.0 provider-threshold evidence bundles for PostgreSQL, SQL Server, MySQL, and Oracle, the checked-in DB2 hotspot evidence bundle, the review-only provider-specific SQL artifact gate, the v0.42.0 provider evidence promotion rules, the v0.42.0 provider tuning gates, and the current provider-read parity evidence posture. It does not create automatic routing, absolute performance guarantees, provider service-level objectives, dashboards, hosted observability, database provisioning, scheduler templates, credential-management guidance, automatic PIT or bridge maintenance, raw SQL or physical-plan promises, deployable provider-specific SQL payload generation, runtime artifact dispatch, package-publication approval, or provider support outside the evidence-linked rows. The coordinated release record for the current provider-bulk, provider-read parity, evidence, tuning, and package documentation baseline is [DVault v0.42.0 Release Notes](releases/v0.42.0.md). Earlier release notes remain historical feature-introduction records.
