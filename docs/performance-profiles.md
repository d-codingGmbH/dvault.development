...
Use [Provider Optimization Gap Matrix](plans/provider-optimization-gap-matrix.md) as the canonical follow-up recommendation surface. Its P0-P3 rows are planning backlog entries: PostgreSQL `latest-satellite-read` remains a capability-gap recommendation, SQL Server, MySQL, Oracle, and DB2 `latest-satellite-read` rows remain evidence-gap recommendations until provider-configured benchmark triplets exist, PostgreSQL and SQL Server `pit-as-of-read` and `bridge-traversal-read` are closed evidence rows when cited through the checked-in v0.32.0 smoke-read bundle, and PostgreSQL, SQL Server, MySQL, Oracle, and DB2 `provider-native-bulk-ingestion` plus MySQL, Oracle, and DB2 remaining `pit-as-of-read` and `bridge-traversal-read` rows remain evidence-gap recommendations until provider-configured benchmark triplets exist. The root quick baseline still preserves skipped PostgreSQL and SQL Server placeholders when their connection strings are unset. Gap-matrix recommendations do not promote skipped placeholders, diagnostics-only rows, smoke-only rows, or storage-footprint rows into completed provider timing evidence.
...
The v0.32.0 smoke read baseline is the authoritative completed-timing bundle for PostgreSQL and SQL Server `pit-as-of-read` and `bridge-traversal-read` rows that selected their provider read strategies. SQL Server latest-satellite closure remains a separate evidence gap, and skipped root quick-baseline rows remain placeholders rather than timing claims.
...
- Provider-read evidence separates completed SQLite latest-satellite, PIT, and bridge timing rows plus completed PostgreSQL and SQL Server PIT/bridge rows in the v0.32.0 smoke-read bundle from root optional-provider read guidance rows that may remain skipped when connection strings are unset.
...
All values in this section are from the root quick benchmark triplet. External-provider completed rows, including SQL Server PIT/bridge timing, should be cited from their linked artifact bundles with preserved run context instead of copied into this quick table:
