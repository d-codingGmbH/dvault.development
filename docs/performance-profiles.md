...
Status: v0.42.0 provider evidence and tuning gate documentation baseline with carried-forward v0.32.0 provider-threshold evidence and v0.31.0 decision-tree contract

This guide is the detailed performance-profile reference for the current DVault performance-guidance baseline. It carries forward the v0.31.0 adopter decision tree, the v0.32.0 provider-threshold evidence bundles for PostgreSQL, SQL Server, MySQL, and Oracle, the review-only provider-specific SQL artifact gate, the v0.42.0 provider evidence promotion rules, the v0.42.0 provider tuning gates, and the current provider-read parity evidence posture. It does not create automatic routing, absolute performance guarantees, provider service-level objectives, dashboards, hosted observability, database provisioning, scheduler templates, credential-management guidance, automatic PIT or bridge maintenance, raw SQL or physical-plan promises, deployable provider-specific SQL payload generation, runtime artifact dispatch, new benchmark artifacts, or package-publication approval. The coordinated release record for the current provider-bulk, provider-read parity, evidence, tuning, and package documentation baseline is [DVault v0.42.0 Release Notes](releases/v0.42.0.md). Earlier release notes remain historical feature-introduction records.
...
Use [Provider Optimization Gap Matrix](plans/provider-optimization-gap-matrix.md) as the canonical follow-up recommendation surface. Its P0-P3 rows are planning and closure entries: PostgreSQL `latest-satellite-read` is a closed capability gap because the checked-in guidance row records `PostgresDataVaultReadStrategy` for supported hub-parent, non-multi-active latest-satellite requests, but that row remains a skipped placeholder until a provider-configured timing lane runs; MySQL `latest-satellite-read` is a closed scoped timing baseline when cited through the ticket `06FE4QQ9VF7B74E60CXEHSS5XW` provider-configured bundle, without claiming a provider-neutral fallback improvement comparator; SQL Server, Oracle, and DB2 `latest-satellite-read` rows remain evidence-gap recommendations until provider-configured benchmark triplets exist; PostgreSQL, SQL Server, MySQL, and Oracle `pit-as-of-read` and `bridge-traversal-read` are closed evidence rows when cited through the checked-in v0.32.0 smoke-read bundle; and PostgreSQL, SQL Server, MySQL, Oracle, and DB2 `provider-native-bulk-ingestion` plus DB2 remaining `pit-as-of-read` and `bridge-traversal-read` rows remain evidence-gap recommendations until provider-configured benchmark triplets exist. The root quick baseline still preserves skipped PostgreSQL, SQL Server, MySQL, Oracle, and DB2 placeholders when their connection strings are unset. Gap-matrix recommendations do not promote skipped placeholders, diagnostics-only rows, smoke-only rows, or storage-footprint rows into completed provider timing evidence.

## v0.42.0 Provider Evidence And Tuning Boundary

The v0.42.0 provider documentation baseline separates measured facts from strategy availability, skipped guidance rows, diagnostics-only posture, smoke-only posture, provider-specific starting gates, and forward-looking recommendations:
...
The v0.32.0 smoke read baseline is the authoritative completed-timing bundle for PostgreSQL, SQL Server, MySQL, and Oracle `pit-as-of-read` and `bridge-traversal-read` rows that selected their provider read strategies. MySQL latest-satellite timing is separate from this PIT/bridge closure and is scoped to the ticket `06FE4QQ9VF7B74E60CXEHSS5XW` bundle. Skipped root quick-baseline rows remain placeholders rather than timing claims.
...
- Provider-read evidence separates completed SQLite latest-satellite, PIT, and bridge timing rows, completed PostgreSQL, SQL Server, MySQL, and Oracle PIT/bridge rows in the v0.32.0 smoke-read bundle, and the completed MySQL latest-satellite row in the ticket `06FE4QQ9VF7B74E60CXEHSS5XW` bundle from root optional-provider read guidance rows that may remain skipped when connection strings are unset.
...
2. Is the request a latest/current or as-of satellite read?
   ...
   Start with the provider-neutral read pipeline and inspect `IDataVaultReadDiagnosticsService` output for request-bound `ReadShape` and read-strategy evidence. SQLite has completed local timing for optimized latest-satellite reads, MySQL has a completed provider-configured optimized latest-satellite timing row for the current window-function path, and PostgreSQL, SQL Server, Oracle, and DB2 have diagnostics-gated latest-satellite strategy selection for supported hub-parent, non-multi-active shapes. Unsupported satellite parents, multi-active unsupported shapes, incomplete `ReadShape` evidence, unknown providers, unregistered providers, and declined strategies must remain provider-neutral fallback unless a later ticket adds new benchmark-backed evidence. Do not claim a MySQL latest-satellite improvement over provider-neutral fallback without a matching comparator row.
...
The checked-in rows focus on `IDataVaultReadService` latest satellite reads, `DataVaultPitAsOfReadRequest`/`DataVaultPitReadRecord`, and `DataVaultBridgeReadRequest`/`DataVaultBridgeReadRecord`. SQLite is the completed optimized latest-satellite timing row in the root benchmark triplet. MySQL latest-satellite completed timing is available from the ticket `06FE4QQ9VF7B74E60CXEHSS5XW` bundle, where `MySqlDataVaultReadStrategy` was selected for the supported current window-function path; that bundle is not a provider-neutral fallback improvement comparator by itself. PostgreSQL PIT/bridge completed timing is available from the v0.32.0 smoke-read bundle, where `PostgresDataVaultReadStrategy` was selected for supported maintained read shapes. The same bundle adds completed SQL Server PIT and bridge timing rows that selected `SqlServerDataVaultReadStrategy`, completed MySQL PIT and bridge timing rows that selected `MySqlDataVaultReadStrategy`, and completed Oracle PIT and bridge timing rows that selected `OracleDataVaultReadStrategy`; latest-satellite timing remains separate from this PIT/bridge closure. DB2 PIT/bridge strategy candidates remain diagnostics-gated repository paths, and new live external-provider execution remains opt-in through provider connection-string configuration.
...
All values in this section are from the root quick benchmark triplet. External-provider completed rows, including MySQL latest-satellite timing and PostgreSQL, SQL Server, MySQL, and Oracle PIT/bridge timing, should be cited from their linked artifact bundles with preserved run context instead of copied into this quick table:
...
Provider-configured PostgreSQL, SQL Server, MySQL, and Oracle PIT/bridge rows should cite [the v0.32.0 smoke-read bundle](../artifacts/benchmarks/v0.32.0-06F9XD26D2MHVAKZ2GCZ67BEFC-smoke-read-20260607/benchmark-summary.md), not the skipped root quick placeholders. Oracle PIT/bridge completed timing evidence is limited to the two Oracle rows in that bundle:
