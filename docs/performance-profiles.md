...
- Fallback and rollback remain part of the claim boundary. Unsupported or mismatched maintenance requests fall back through provider-neutral maintenance. PostgreSQL caller transactions also fall back because the current provider strategy does not create a strategy-owned savepoint, and SQL Server full-rebuild faults or cancellations preserve the pre-rebuild rows.

MySQL, Oracle, and DB2 PIT maintenance claims must not cite completed `pit-as-of-read` rows as maintenance proof. Future accepted ordinary hub-parent implementation lanes must prove provider-name gating, clean-context gating, complete maintenance-shape evidence, rollback-clean delete-plus-insert execution when the strategy owns the transaction, and fallback when an ambient caller transaction cannot provide a verified savepoint or equivalent rollback boundary.
