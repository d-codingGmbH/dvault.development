...
For the current v0.41.0 compatibility baseline, keep `DCoding.Data.DVault.Analyzers` on one `net10.0` analyzer asset and treat the `.NET 10 SDK` as the supported build-host baseline for both the visible `8.41.0` and `10.41.0` package lines.
...
- `tools/pack-release-packages.sh` packs the analyzer project once for `8.41.0` and once for `10.41.0` without changing the analyzer target framework, so both package lines currently carry the same `net10.0` analyzer binary with different package versions.
...
- Make the analyzer build-host requirement explicit anywhere `8.41.0` analyzer installation guidance appears if the package is meant to stay `net10.0`-only.
...
- Keep package verification and install guidance aligned with whichever compatibility claim is accepted so the `8.41.0` analyzer package is not documented more broadly than it is verified.
