...
Do not publish or document a consumer-facing `0.38.0` DVault package version from the v0.38.0 release label. Do not combine `8.38.0` and `10.38.0` packages in one published artifact family or consumer example.
...
- Release baseline `v0.38.0` uses the dual package-line compatibility contract in this document: `8.38.0` for `net8.0` and EF Core 8, `10.38.0` for `net10.0` and EF Core 10, eight coordinated package IDs including `DCoding.Data.DVault.Db2`, no consumer-facing `0.38.0` package line, no mixed-line restored targets, DB2 execution documented as optimized save plus PIT/bridge read dispatch without latest-satellite optimization or live-schema reading, stable hash algorithm selection documented as a caller-owned compatibility decision, binary hash-key storage documented as explicit opt-in physical storage rather than an automatic migration, and analyzer consumption documented against the `.NET 10 SDK` build-host baseline.
