...
The current v0.50.0 package compatibility baseline continues the visible dual consumer package-version contract from v0.36.0, advances the package lines to `8.50.0` and `10.50.0`, carries forward the DB2 provider package baseline, stable hash algorithm-selection guidance, binary hash-key storage adoption guidance, and analyzer build-host guidance, keeps the optional privacy extension package in the coordinated family, documents repeated same-hub generated mapper parity for explicit unique produced participant names, and aligns target-specific dependency pins, release-note links, and package verification documentation. It does not by itself publish packages, provision external databases, create release automation, change the default stable hash algorithm or storage profile, add automatic persisted-hash migration, add dependent child key modeling, add compliance automation, own key lifecycle management, or split the DVault library into a platform/tool-suite surface. DB2 execution claims stay limited to the registered clean-context save strategy, diagnostics-gated PIT/bridge read strategy, opt-in live smoke evidence documented for the v0.34.0 DB2 baseline, and external opt-in live-schema evidence owned by the consuming environment. The MySQL PIT maintenance lane is source/test-backed for official-provider ordinary hub-parent full rebuilds only and is not benchmark-backed maintenance timing evidence.
...
Do not publish or document a consumer-facing `0.50.0` DVault package version from the v0.50.0 release label. Do not combine `8.50.0` and `10.50.0` packages in one published artifact family or consumer example.
...
Downstream package verification, matrix tests, release notes, README guidance, and CI documentation are incomplete if they blur release label `v0.50.0` with package versions `8.50.0` and `10.50.0`, omit one of the required EF/provider pins above, omit the DB2 or Privacy package from the nine-package family, allow a mixed 8.x/10.x restored target, overstate DB2 beyond the registered save and PIT/bridge read strategies, overstate generated same-hub link mapper support beyond explicit unique produced participant names, overstate dependent child key modeling as shipped, overstate stable hash opt-in ids as security or compliance controls, overstate binary hash-key storage as an automatic migration or public byte-key contract, overstate privacy extension seams as compliance automation or provider-native encryption, overstate analyzer support beyond the `.NET 8 SDK` and `.NET 10 SDK` build-host baseline, or export analyzer assets as runtime dependencies.
...
- Release baseline `v0.50.0` uses the dual package-line compatibility contract in this document: `8.50.0` for `net8.0` and EF Core 8, `10.50.0` for `net10.0` and EF Core 10, nine coordinated package IDs including `DCoding.Data.DVault.Db2` and optional `DCoding.Data.DVault.Privacy`, no consumer-facing `0.50.0` package line, no mixed-line restored targets, repeated same-hub generated link mapper support only for explicit unique produced participant names, dependent child key modeling deferred, DB2 execution documented as optimized save plus latest-satellite/PIT/bridge read dispatch with external opt-in live-schema evidence and current 2026-06-23 closure timing, MySQL PIT maintenance documented as source/test-backed official-provider ordinary hub-parent full-rebuild behavior without timing claims, stable hash algorithm selection documented as a caller-owned compatibility decision, binary hash-key storage documented as explicit opt-in physical storage rather than an automatic migration, privacy extension behavior documented as explicit opt-in registration and encrypted-payload conversion seams rather than compliance automation, and analyzer consumption documented against the `.NET 8 SDK` and `.NET 10 SDK` build-host baseline.
