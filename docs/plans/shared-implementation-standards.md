...
The current v0.100.0 package compatibility baseline continues the visible dual consumer package-version contract from v0.36.0, advances the package lines to `8.100.0` and `10.100.0`, carries forward the DB2 provider package baseline, stable hash algorithm-selection guidance, analyzer build-host guidance, and optional privacy package boundary, and records the breaking binary physical hash-key storage default. It keeps the optional privacy extension package in the coordinated family, documents repeated same-hub generated mapper parity for explicit unique produced participant names, adds dependent child key modeling support, adds satellite effectivity role metadata, and aligns target-specific dependency pins, release-note links, and package verification documentation. It does not by itself publish packages, provision external databases, create release automation, add automatic persisted-hash migration, add compliance automation, own key lifecycle management, or split the DVault library into a platform/tool-suite surface. DB2 execution claims stay limited to the registered clean-context save strategy, diagnostics-gated latest-satellite/PIT/bridge read strategy, external opt-in live-schema evidence owned by the consuming environment, and the checked-in v0.100.0 binary-default benchmark evidence. The MySQL PIT maintenance lane is source/test-backed for official-provider ordinary hub-parent full rebuilds only and is not benchmark-backed maintenance timing evidence.

The coordinated package family contains these nine package IDs across all compatibility lines:
...
Do not publish or document a consumer-facing `0.100.0` DVault package version from the v0.100.0 release label. Do not combine `8.100.0` and `10.100.0` packages in one published artifact family or consumer example. Mark both package lines as a breaking release because generated hash-key and participant-reference columns now default to binary physical storage.
...
`DCoding.Data.DVault.Privacy` remains optional and opt-in. It may expose provider-neutral registration, alias configuration, and encrypted payload conversion seams, but it must not claim GDPR/DSGVO compliance, automatic encryption, pseudonymization, redaction, deletion, retention completion, key storage, key rotation, key destruction, provider-native encryption DDL, or provider-native encryption runtime dispatch unless a later explicit ticket adds and verifies that contract.

Downstream package verification, matrix tests, release notes, README guidance, and CI documentation are incomplete if they blur release label `v0.100.0` with package versions `8.100.0` and `10.100.0`, omit one of the required EF/provider pins above, omit the DB2 or Privacy package from the nine-package family, allow a mixed 8.x/10.x restored target, overstate DB2 beyond the registered save and PIT/bridge read strategies, overstate generated same-hub link mapper support beyond explicit unique produced participant names, overstate dependent child key support beyond declared DCK metadata and bindings, overstate satellite effectivity as automatic effectivity logic, overstate stable hash opt-in ids as security or compliance controls, overstate binary hash-key storage as an automatic migration or public byte-key contract, overstate privacy extension seams as compliance automation or provider-native encryption, overstate analyzer support beyond the `.NET 8 SDK` and `.NET 10 SDK` build-host baseline, or export analyzer assets as runtime dependencies.
...
- Current packable runtime/provider projects target `net8.0;net10.0`; the analyzer package targets one `netstandard2.0` asset under `analyzers/dotnet/cs/`; tooling, benchmark, and repository helper projects may stay on `net10.0` when they are not consumer runtime packages.
- Release baseline `v0.100.0` uses the dual package-line compatibility contract in this document: `8.100.0` for `net8.0` and EF Core 8, `10.100.0` for `net10.0` and EF Core 10, nine coordinated package IDs including `DCoding.Data.DVault.Db2` and optional `DCoding.Data.DVault.Privacy`, no consumer-facing `0.100.0` package line, no mixed-line restored targets, repeated same-hub generated link mapper support for explicit unique produced participant names, dependent child key metadata/binding support, satellite effectivity role metadata, DB2 execution documented as optimized save plus latest-satellite/PIT/bridge read dispatch with external opt-in live-schema evidence and current v0.100.0 binary-default closure timing, MySQL PIT maintenance documented as source/test-backed official-provider ordinary hub-parent full-rebuild behavior without timing claims, stable hash algorithm selection documented as a caller-owned compatibility decision, binary hash-key storage documented as the current default physical storage rather than an automatic migration, privacy extension behavior documented as explicit opt-in registration and encrypted-payload conversion seams rather than compliance automation, and analyzer consumption documented against the `.NET 8 SDK` and `.NET 10 SDK` build-host baseline.
