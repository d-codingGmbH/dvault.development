...
- Release baseline `v0.45.0` uses the dual package-line compatibility contract in this document: `8.45.0` for `net8.0` and EF Core 8, `10.45.0` for `net10.0` and EF Core 10, nine coordinated package IDs including `DCoding.Data.DVault.Db2` and optional `DCoding.Data.DVault.Privacy`, no consumer-facing `0.45.0` package line, no mixed-line restored targets, DB2 execution documented as optimized save plus latest-satellite/PIT/bridge read dispatch with external opt-in live-schema evidence and scoped DB2 hotspot timing, stable hash algorithm selection documented as a caller-owned compatibility decision, binary hash-key storage documented as explicit opt-in physical storage rather than an automatic migration, privacy extension behavior documented as explicit opt-in registration and encrypted-payload conversion seams rather than compliance automation, and analyzer consumption documented against the `.NET 10 SDK` build-host baseline.
