...
Provider schema guardrails stay tied to the finite v0.29.0 supported-provider baseline: SQLite (`sqlite-v1`), Oracle (`oracle-v1`), PostgreSQL (`postgres-v1`), SQL Server (`sqlserver-v1`), and MySQL (`mysql-pomelo-v1`). Logical names in the reviewed `dvault.model.v1` artifact remain provider-neutral; provider profiles may derive safe physical names only for DVault-owned generated metadata when identifier length, reserved-word, escaping, included-index, duplicate-index, or collision rules require it. Unrecognized providers do not inherit provider-specific DDL safety guarantees from the supported profiles.
