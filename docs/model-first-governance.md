...
Typed satellite read-model generation consumes the reviewed support bundle as exactly one authoritative additional file with `DVaultGenerateTypedReadModels=true`. Pin `DVaultTypedReadModelMetadataSourceFingerprint` when the consuming build should fail on metadata-source drift. The generator reads the projected support-bundle explain descriptor; it does not parse raw `dvault.model.v1` JSON additional files, source-visible Code-First callbacks, or literal metadata-first declarations directly.

Generated helpers are limited to hub-parent, link-parent, and deterministic multi-active satellites. PIT and bridge declarations remain model-first or runtime-service metadata that can be validated, projected, maintained, and read through the documented services, but they do not produce typed read-model helpers in the current generator boundary.
