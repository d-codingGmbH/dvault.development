...
Status: v0.29.0 public guidance

This guide describes how teams should use governed `dvault.model.v1` JSON artifacts alongside the existing Code-First and metadata-first DVault paths. Earlier release notes remain historical package context; `docs/releases/v0.29.0.md` is the current public baseline for provider schema guardrail guidance, finite supported-provider DDL safety claims, Code-First parity, analyzer/generator ergonomics, EF misuse diagnostics, model-first import, export, projection, design-time drift, explicit snapshot-model preflight, optional live-schema drift guidance, migration guardrail outcomes, idempotency preflight, aggregate preflight, explicit bulk ingestion, provider benchmark evidence, compiled-model/query/pooling evidence boundaries, bounded query-shape guidance, explicit PIT/bridge maintenance, current/as-of convenience reads, SQLite PIT/bridge read optimization, opt-in save/read telemetry, redacted support-bundle export, support-bundle-driven typed satellite/PIT/bridge read-model generation, stable `sha256-v1` hash governance, chunked explicit-save guidance, async source save guidance, and staged provider bulk ingestion guidance carried forward from earlier release notes.
...
Use model-first governance when the authoritative model should be a reviewed, versioned `dvault.model.v1` JSON artifact. This path is intended for source-controlled artifact reviews, strict JSON import diagnostics, projection into EF metadata, canonical JSON export from fluent Code-First declaration callbacks or already-materialized metadata, drift reports used as review evidence, and consumer-owned support-bundle export after projection. A raw `dvault.model.v1` file is not a typed read-model generator input until it has been imported, projected, and represented in an authoritative `dvault.support-bundle.v1` diagnostics artifact.
...
Canonical v1 JSON uses the stable top-level declaration categories `hubs`, `links`, `satellites`, `pits`, and `bridges`, with `naming.policy` defaulting to `default` and `loadTimestampStorage` defaulting to `provider-default`. The supported `loadTimestampStorage` tokens are `provider-default`, `iso-8601-utc-text`, and `utc-ticks`. Unknown fields are errors at every object level, including nested `naming`, parent reference, participant, and bridge endpoint objects. Review declaration array order as part of the model because import, export, projection, and drift evidence preserve stable declaration ordering. The runtime `DataVaultPitMetadata` path can maintain and read bounded link-parent PITs, but `dvault.model.v1` PIT artifacts remain hub-parent-only and continue to use the `hub` field.
...
When a reachable database must be checked, use the separate bounded live-schema path: `DataVaultLiveSchemaReader.ReadAsync(context)` captures DVault-owned tables, ordered columns, named primary-key constraints, and secondary indexes, and `DataVaultLiveSchemaDriftReporter.Compare` compares that result with expected metadata or an imported model. Built-in reader coverage includes SQLite, PostgreSQL, SQL Server, Oracle, DB2, and MySQL. `IBM.EntityFrameworkCore` maps to the DB2 reader; both `MySql.EntityFrameworkCore` and `Pomelo.EntityFrameworkCore.MySql` map to the MySQL reader. Providers without a built-in reader return `UnsupportedProvider`, and reachable-provider failures return `Unavailable`, so requested live checks do not silently pass.
...
Provider schema guardrails stay tied to the finite supported-provider baseline: SQLite (`sqlite-v1`), Oracle (`oracle-v1`), PostgreSQL (`postgres-v1`), SQL Server (`sqlserver-v1`), DB2 (`db2-v1`), and MySQL (`mysql-pomelo-v1`). Logical names in the reviewed `dvault.model.v1` artifact remain provider-neutral; provider profiles may derive safe physical names only for DVault-owned generated metadata when identifier length, reserved-word, escaping, included-index, duplicate-index, or collision rules require it. Unrecognized providers do not inherit provider-specific DDL safety guarantees from the supported profiles.

Review provider-specific DDL outcomes before applying migrations. A long logical name can remain the produced name while a MySQL physical identifier must fit the 64-character profile limit or fail before unsafe DDL is emitted. An Oracle migration should not recreate a secondary index that the provider profile omitted because the primary key already covers the effective key. In both cases, the adopter response is to rename the source declaration or role, keep the EF migration aligned with the generated DVault metadata, change the reviewed `loadTimestampStorage` token only when that is intentional, or choose a supported profile that can represent the shape.

## Support-Bundle And Typed Helper Workflow

The support-bundle path is a consumer-owned export flow layered after model review and projection. For model-first usage, review the `dvault.model.v1` JSON artifact, import it with `DataVaultModelArtifactImporter.ImportJson`, configure the design-time context with `UseDataVaultMetadata(importResult)`, run validation or drift checks, and then invoke the consumer-hosted `support-bundle` command:

```sh
dotnet run --project src/SalesVault/SalesVault.csproj -- support-bundle --artifact models/sales-vault.json --output src/SalesVault/dvault-support-bundle.json
```

The exported `dvault.support-bundle.v1` document is not a repository-wide baseline file and is not produced by a standalone DVault CLI. The consumer project owns the command host, output path, review policy, storage, attachment, transport, retention, and any approval workflow. Use `--live-schema` only when that project also owns the reachable database, credentials, lifecycle cleanup, and CI isolation for the live check.

Typed read-model generation consumes the reviewed support bundle as exactly one authoritative additional file with `DVaultGenerateTypedReadModels=true`. Pin `DVaultTypedReadModelMetadataSourceFingerprint` when the consuming build should fail on metadata-source drift. The generator reads the projected support-bundle explain descriptor; it does not parse raw `dvault.model.v1` JSON additional files, source-visible Code-First callbacks, or literal metadata-first declarations directly.

Generated helpers are limited to support-bundle-proven satellite latest/current/as-of reads, PIT as-of reads, and bounded bridge traversal reads. PIT and bridge helper generation requires representative request-bound `ReadShape` diagnostics in the support bundle; model-first PIT and bridge declarations can be validated, projected, maintained, and read through the documented services, but raw model-first declarations alone do not produce typed helper emission.
...
| `DMV1303` | `reference` | PIT satellite parent mismatch | Reference only satellites whose parent hub matches the model-first PIT hub. |
...
The current baseline provides reusable library-hosted design-time command verbs through a consumer-owned command host, but it does not ship a standalone DVault CLI, intercept `dotnet ef`, apply migrations, or repair schema drift automatically. The command verbs are `validate`, `export`, `drift`, `guardrail`, and `support-bundle`; `export` is for artifact maintenance and reviewed refresh workflows, not the default blocking CI gate. `DataVaultPreflight.Run(...)` can aggregate validation/provider explain, reviewed-artifact drift, explicit snapshot-model drift, migration guardrail, and representative request diagnostics when the consumer supplies those inputs. The `guardrail --migration` lane blocks destructive drops of DVault-produced tables, generated columns, generated secondary indexes, and named generated primary-key constraints; explicit EF rename operations remain intentional risky operations, and suspicious drop-plus-add replacements are reported as metadata or produced-name drift for review. `support-bundle` emits a deterministic redacted `dvault.support-bundle.v1` JSON artifact from the consumer-owned design-time host, with reviewed-artifact and live-schema sections remaining explicit opt-ins. Direct YAML ingestion, automatic snapshot discovery, automatic reviewed-artifact discovery, automatic support-bundle routing, automatic representative request generation, and extraction from arbitrary EF `ModelBuilder` state into a model artifact remain outside the current contract.

Provider schema guardrails are design-time review and diagnostics boundaries for DVault-owned generated metadata. They do not add provider-specific guarantees outside SQLite, Oracle, PostgreSQL, SQL Server, DB2, and MySQL; they do not silently map unrecognized providers to supported profiles; and they do not repair migrations, execute migrations, rewrite raw SQL, or rewrite arbitrary consumer-authored DDL.

The model-first APIs operate on JSON artifacts, fluent Code-First declaration callbacks, and already-materialized metadata. The current `dvault.model.v1` artifact contract can represent repeated same-hub participant roles and link-parent satellites, including effectivity modeled as ordinary caller-owned link-parent satellite state. It does not represent link-parent PIT declarations; link-parent PIT maintenance and reads are limited to the existing `DataVaultPitMetadata` / registry-backed runtime maintenance path documented in [DVault V1 PIT And Bridge Boundary](architecture/dvault-v1-pit-bridge-boundary.md). Registry-backed PIT coverage remains maintenance-request resolution only and does not include a registry-backed PIT as-of read request. The typed read-model generator does not parse raw model-first artifacts directly and does not infer PIT or bridge helpers from raw model-first declarations alone. Attribute-driven generated link mappers can cover repeated same-hub links only when the source declaration supplies explicit unique produced participant names that match the projected role-bearing metadata; raw model-first support-bundle mapper generation remains outside the current public claim set. PIT and bridge helper emission requires an authoritative support bundle with request-bound `ReadShape` evidence after model projection. Dependent child key modeling, model-first same-hub typed mapper generation, and effectivity-specific fluent APIs remain outside the current public claim set. Keep standalone CLI packaging, YAML semantics, database provisioning, secret-management recipes, and EF model reverse-engineering as separate future contracts instead of implying support in current examples.
