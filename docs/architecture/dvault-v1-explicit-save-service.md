...
- `src/DCoding.Data.DVault.Oracle`: an Oracle-gated insert-only hub/link save strategy exposed by `AddDVaultOracle()` for clean `Oracle.EntityFrameworkCore` batches, with dirty tracked contexts and satellite request batches declined for provider-neutral fallback.
