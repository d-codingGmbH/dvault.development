...
`AddDVaultSqlite()`, `AddDVaultSqlServer()`, `AddDVaultOracle()`, and `AddDVaultDb2()` register repository-proven diagnostics-gated optimized latest-satellite, PIT, and bridge read strategy candidates for their supported shapes. `AddDVaultPostgres()` and `AddDVaultMySql()` register repository-proven diagnostics-gated optimized PIT/bridge read strategy candidates. Unsupported providers, PostgreSQL/MySQL latest-satellite requests, declined request shapes, incomplete read-shape evidence, and stale PIT/bridge maintenance evidence keep the provider-neutral read pipelines. Neither read path performs maintenance, schedules background work, updates rows during `SaveChanges`, or turns PIT/bridge metadata into automatic orchestration.
...
The public read request contract is provider-neutral. `AddDVaultSqlite()`, `AddDVaultSqlServer()`, `AddDVaultOracle()`, and `AddDVaultDb2()` register optimized read dispatch for supported hub-parent, non-multi-active latest-satellite reads plus supported PIT and bridge read shapes. `AddDVaultPostgres()` and `AddDVaultMySql()` register optimized PIT and bridge read strategy candidates for PostgreSQL and MySQL without adding latest-satellite read dispatch. `AddDVault()` without a provider-specific read strategy, unsupported providers, PostgreSQL/MySQL latest-satellite requests, declined request shapes, incomplete generated read-model projection evidence, and observable stale-maintenance signals such as pending tracked changes fall back to the provider-neutral read path.
...
- [DataVaultProviderReadStrategyTests.cs](../../tests/DCoding.Data.DVault.Tests/Unit/DataVaultProviderReadStrategyTests.cs) covers SQL Server, Oracle, and DB2 latest-satellite gates plus PostgreSQL, SQL Server, MySQL, Oracle, and DB2 PIT/bridge candidate gates for provider match, supported shape selection, unsupported-shape fallback, incomplete-evidence fallback, and stale-maintenance-signal fallback causes.
...
The shared benchmark artifact contract is [Performance Evidence And Benchmark Artifact Contract](../plans/performance-evidence-benchmark-artifact-contract.md). The relevant completed timing rows are SQLite `latest-satellite-read`, `pit-as-of-read`, and `bridge-traversal-read`. Optional PostgreSQL, SQL Server, MySQL, Oracle, and DB2 read rows remain visible as skipped rows when their connection-string environment variables are unset; SQL Server, Oracle, and DB2 latest-satellite/PIT/bridge rows plus the other providers' PIT/bridge rows carry `selectedStrategy`/`plannedReadStrategy` guidance. PostgreSQL and MySQL latest-satellite rows explicitly record that no provider-specific latest-satellite read strategy is registered.
...
- PostgreSQL and MySQL optimized latest-satellite read claims.
