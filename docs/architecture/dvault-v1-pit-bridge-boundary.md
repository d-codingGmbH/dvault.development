...
Current package/adoption baseline: [DVault v0.100.0 Release Notes](../releases/v0.100.0.md)
Provider maintenance baseline: [DVault v0.47.0 Release Notes](../releases/v0.47.0.md)
...
- `AddDVaultMySql()` registers `IDataVaultProviderPitMaintenanceStrategy` through `MySqlDataVaultPitMaintenanceStrategy`. The default PIT maintenance service can select that provider strategy only for clean official `MySql.EntityFrameworkCore` full rebuilds of ordinary hub-parent PITs.
- PostgreSQL, SQL Server, and MySQL provider paths all stay request-gated. Provider-name mismatch, dirty tracked contexts, unsupported PIT shapes, incomplete provider evidence, PostgreSQL caller transactions, MySQL caller transactions without a verified rollback-clean savepoint boundary, and provider-specific guard failures fall back to the provider-neutral maintenance implementation.
- PostgreSQL caller transactions use provider-neutral PIT maintenance because the PostgreSQL provider strategy does not currently create a strategy-owned savepoint around delete-plus-insert full rebuild work. The fallback cause is `CurrentTransactionSavepointUnavailable`, so `AddDVaultPostgres()` does not silently claim rollback-clean optimized rebuild behavior inside an ambient transaction.
...
- MySQL `MaintainParentsAsync(...)`, Pomelo-backed maintenance, MySQL multi-active PITs, MySQL link-parent PITs, and MySQL caller transactions that cannot provide rollback-clean savepoint behavior use provider-neutral maintenance. MySQL full-rebuild faults and cancellations preserve the pre-rebuild rows when the MySQL strategy owns the local transaction or a verified savepoint boundary.

No provider registration turns PIT maintenance into automatic work. Applications still decide when rebuilds or parent maintenance run, and PIT reads still consume only rows already maintained before the read request. Completed PIT read rows cannot be cited as provider-maintenance timing or as proof that provider SQL maintained those rows.

### MySQL Full-Rebuild Push-Down Evaluation

Ticket `06FF43CJ9CJMG7J917RW22QKJC` evaluated MySQL PIT maintenance push-down for `IDataVaultPitMaintenanceService.RebuildAsync(...)` full rebuilds only. The current repository baseline proves that `AddDVaultMySql()` registers MySQL save plus latest-satellite, PIT, and bridge read strategy candidates for both supported EF Core provider names, `Pomelo.EntityFrameworkCore.MySql` and `MySql.EntityFrameworkCore`. Ticket `06FFDG522514HX2J17GT9VE77W` adds the initial MySQL maintenance lane through `MySqlDataVaultPitMaintenanceStrategy` for the official `MySql.EntityFrameworkCore` provider only. The checked-in MySQL PIT read timing still proves optimized reads over already-maintained PIT rows; it is not maintenance push-down timing evidence.

The implemented MySQL lane is a MySQL `IDataVaultProviderPitMaintenanceStrategy`, not a SQL Server-style service replacement. The existing strategy seam dispatches only full rebuilds and preserves provider-neutral `MaintainParentsAsync(...)` fallback, which matches the evaluated MySQL scope. The MySQL gate evaluator reports provider-name mismatch, unknown or unregistered provider names, dirty tracked context, incomplete maintenance-shape evidence, unsupported PIT shape, and rollback/savepoint boundary fallback causes.

The accepted implementation is deliberately narrow: clean ordinary hub-parent MySQL full rebuilds only, using the live `MySql.EntityFrameworkCore` package surface as the repository-proven provider lane. Pomelo remains capability/profile-supported but not live-maintenance-proven. Shared-driving-key multi-active hub-parent PIT full rebuilds and link-parent non-multi-active PIT full rebuilds stay deferred for MySQL because only PostgreSQL currently proves those maintenance shapes through source and live integration evidence. MySQL `MaintainParentsAsync(...)`, link-parent multi-active PITs, incompatible driving-key-family PITs, bridge maintenance, automatic maintenance, read-time refresh, dirty contexts, provider mismatches, and caller transactions without a verified rollback-clean savepoint boundary are rejected for the initial MySQL maintenance lane.

Rollback is the decisive gate for the accepted MySQL lane. The MySQL provider strategy preserves pre-rebuild PIT rows when it owns a local transaction and rolls back delete-plus-insert work on fault or cancellation. If a caller transaction is already active, the MySQL path requires EF Core-reported savepoint support for the active provider and rolls back to a strategy-owned savepoint on failure; otherwise it falls back to provider-neutral maintenance.

### MySQL, Oracle, And DB2 PIT Maintenance Decision Matrix

MySQL now has the accepted narrow provider-maintenance lane for official `MySql.EntityFrameworkCore` ordinary hub-parent full rebuilds. Oracle and DB2 still register save plus latest-satellite, PIT-read, and bridge-read strategies but no provider-specific PIT maintenance strategy or service replacement. Completed PIT read timing for these providers is read-side evidence over already-maintained PIT rows and is not write-side PIT maintenance proof.

| Provider | Decision | Accepted lane or child | Deferred shapes | Fallback-only or rejected shapes |
| --- | --- | --- | --- | --- |
| MySQL | Implemented one ordinary hub-parent full-rebuild lane through `IDataVaultProviderPitMaintenanceStrategy`. | Ticket `06FFDG522514HX2J17GT9VE77W` adds official `MySql.EntityFrameworkCore` ordinary hub-parent `RebuildAsync(...)` push-down. | Pomelo live validation, shared-driving-key multi-active hub-parent full rebuilds, link-parent non-multi-active full rebuilds, and benchmark-backed maintenance timing. | `MaintainParentsAsync(...)`, link-parent multi-active PITs, incompatible driving-key-family PITs, bridge maintenance, automatic maintenance, read-time refresh, dirty contexts, provider mismatches, incomplete maintenance-shape evidence, and caller transactions without verified savepoints. |
...
Bridge rebuild and maintenance push-down stays deferred from this boundary. The current provider-specific maintenance work is limited to PIT maintenance: PostgreSQL contributes `PostgresDataVaultPitMaintenanceStrategy`, SQL Server replaces `IDataVaultPitMaintenanceService` with `SqlServerDataVaultPitMaintenanceService` for a narrower ordinary hub-parent full-rebuild gate, and MySQL contributes `MySqlDataVaultPitMaintenanceStrategy` for the official-provider ordinary hub-parent full-rebuild gate. The MySQL lane does not add bridge maintenance scope. Bridge maintenance remains the provider-neutral `IDataVaultBridgeMaintenanceService` surface, and this contract does not expose an `IDataVaultProviderBridgeMaintenanceStrategy` counterpart.
...
- [PostgresProviderCapabilityTests.cs](../../tests/DCoding.Data.DVault.Tests/Unit/PostgresProviderCapabilityTests.cs) covers PostgreSQL PIT maintenance strategy registration, the approved full-rebuild gate for ordinary hub-parent, shared-driving-key multi-active hub-parent, and link-parent non-multi-active PITs, and the caller-transaction savepoint fallback cause.
- [PostgresPitMaintenanceServiceTests.cs](../../tests/DCoding.Data.DVault.Tests/Integration/PostgresPitMaintenanceServiceTests.cs) covers configured PostgreSQL full rebuild behavior for the supported PIT shapes and provider-neutral fallback when a caller transaction is already active.
- [MySqlProviderCapabilityTests.cs](../../tests/DCoding.Data.DVault.Tests/Unit/MySqlProviderCapabilityTests.cs) covers MySQL PIT maintenance strategy registration, the official-provider ordinary hub-parent full-rebuild gate, Pomelo fallback, deferred-shape fallback, savepoint-boundary fallback, and generated MySQL command-plan SQL shape.
- [MySqlPitMaintenanceServiceTests.cs](../../tests/DCoding.Data.DVault.Tests/Integration/MySqlPitMaintenanceServiceTests.cs) covers configured official-provider MySQL ordinary PIT rebuild behavior plus rollback-clean local transaction behavior on failure and cancellation.
...
- [MySqlProviderCapabilityTests.cs](../../tests/DCoding.Data.DVault.Tests/Unit/MySqlProviderCapabilityTests.cs) also covers MySQL latest-satellite read strategy registration and generated window-function SQL shape.
...
- [DVaultMySqlServiceCollectionExtensions.cs](../../src/DCoding.Data.DVault.MySql/DVaultMySqlServiceCollectionExtensions.cs) registers MySQL save and read strategy candidates plus `MySqlDataVaultPitMaintenanceStrategy` for the official-provider ordinary hub-parent maintenance lane.
...
The shared benchmark artifact contract is [Performance Evidence And Benchmark Artifact Contract](../plans/performance-evidence-benchmark-artifact-contract.md). The relevant completed timing rows include SQLite `latest-satellite-read`, `pit-as-of-read`, and `bridge-traversal-read` from the root quick triplet plus PostgreSQL, SQL Server, MySQL, Oracle, and DB2 latest-satellite, PIT, and bridge rows from the [2026-06-23 provider optimization closure bundle](../../artifacts/benchmarks/06FF0000000000000000000000-provider-optimization-closure-20260623/README.md). Those provider-configured rows selected `PostgresDataVaultReadStrategy`, `SqlServerDataVaultReadStrategy`, `MySqlDataVaultReadStrategy`, `OracleDataVaultReadStrategy`, and `Db2DataVaultReadStrategy` for supported latest-satellite and maintained PIT/bridge shapes. For PIT and bridge, the maintained read-model rows are prerequisites for the read timing row, not maintenance work included in the measured timing. Optional PostgreSQL, SQL Server, MySQL, Oracle, and DB2 root quick read rows remain visible as skipped rows when their connection-string environment variables are unset; those root placeholders preserve row identity and planned strategy facts but are not completed external-provider timing evidence.

Use [Provider Optimization Evidence Matrix](../plans/provider-optimization-evidence-matrix.md) as the canonical read-row and PIT full-rebuild maintenance-row lookup for scenario, provider, baseline, evidence posture, authoritative artifact source, and finite stop/fallback conditions. This PIT/bridge boundary remains the behavior contract; the matrix is the citation surface for downstream provider optimization evidence.
...
- Provider-specific PIT maintenance beyond the current PostgreSQL strategy, SQL Server service-replacement gate, and official `MySql.EntityFrameworkCore` ordinary hub-parent full-rebuild gate; DB2 remains an accepted future ordinary hub-parent lane, and Oracle remains deferred.
