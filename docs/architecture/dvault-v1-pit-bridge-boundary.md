...
The shared benchmark artifact contract is [Performance Evidence And Benchmark Artifact Contract](../plans/performance-evidence-benchmark-artifact-contract.md). The relevant completed timing rows are SQLite `latest-satellite-read`, `pit-as-of-read`, and `bridge-traversal-read`. Optional PostgreSQL, SQL Server, MySQL, Oracle, and DB2 read rows remain visible as skipped rows when their connection-string environment variables are unset; SQL Server, MySQL, Oracle, and DB2 latest-satellite rows plus all PIT/bridge rows carry `selectedStrategy`/`plannedReadStrategy` guidance, while PostgreSQL latest-satellite rows explicitly record that no provider-specific latest-satellite read strategy is registered.
