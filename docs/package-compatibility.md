...
The analyzer package currently ships one `net10.0` analyzer asset for both coordinated package lines. Supported analyzer consumption for both `8.49.0` and `10.49.0` uses a `.NET 10 SDK` build host, including `net8.0` projects on the `8.49.0` package line. The repository does not validate pure `.NET 8 SDK` analyzer consumption.
