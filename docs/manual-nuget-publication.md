...
The v0.37.0 baseline is target-specific. `8.36.0` / `net8.0` uses the EF Core 8 dependency line, and `10.36.0` / `net10.0` uses the EF Core 10 dependency line. Patch movement is allowed only within the selected target major line and must be reflected together in the project files, matrix tests, package verifier, release notes, and adopter guidance.
