...
The current v0.41.0 documentation baseline uses two consumer package-version lines over these package ids:
...
Do not publish or document a consumer-facing `0.41.0` package version from the v0.41.0 release label. Do not combine `8.41.0` and `10.41.0` packages in one consumer install example or one publish approval.
...
- package verification confirms the packaged README guidance separates `8.41.0` for `net8.0` and EF Core 8 from `10.41.0` for `net10.0` and EF Core 10
- release notes and README guidance explain that `v0.41.0` is the release tag and release-note label, not a consumer NuGet package version, and keep the dependency-line, analyzer-compatibility, manual-publication, and validation guidance aligned with the published package-version lines
...
## Current v0.41 Dependency Matrix

The v0.41.0 baseline is target-specific. `8.41.0` / `net8.0` uses the EF Core 8 dependency line, and `10.41.0` / `net10.0` uses the EF Core 10 dependency line. Patch movement is allowed only within the selected target major line and must be reflected together in the project files, matrix tests, package verifier, release notes, and adopter guidance.
...
2. Select one package-version line for this approval: `8.41.0` for `net8.0` and EF Core 8, or `10.41.0` for `net10.0` and EF Core 10.
