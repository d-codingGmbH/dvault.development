...
public sealed class DataVaultPitReadServiceTests {
  ...
  [Fact]
  public async Task PitReadRejectsContradictingMultiActiveReferenceBeforeRowMaterialization() {
    var readService = new DefaultDataVaultReadService();
  ...
  private sealed class ContradictingMultiActiveReferenceContext(DbContextOptions<ContradictingMultiActiveReferenceContext> options) : DbContext(options) {
    protected override void OnModelCreating(ModelBuilder modelBuilder) {
      ...
      ConfigureOrdinaryProfileSatellite(modelBuilder);
    }

    private static void ConfigureOrdinaryProfileSatellite(ModelBuilder modelBuilder) {
      modelBuilder.SharedTypeEntity<Dictionary<string, object>>("SatCustomerProfile", entityBuilder => {
        ...
        var hashDiff = entityBuilder.IndexerProperty<string>("HashDiff");
        hashDiff.Metadata.SetAnnotation(DataVaultAnnotationNames.PropertyRole, DataVaultPropertyRole.Technical);
        hashDiff.Metadata.SetAnnotation(DataVaultAnnotationNames.TechnicalColumnRole, TechnicalMetadataColumnRole.HashDiff);

        var loadTimestamp = entityBuilder.IndexerProperty<DateTimeOffset>("LoadTimestamp");
        loadTimestamp.Metadata.SetAnnotation(DataVaultAnnotationNames.PropertyRole, DataVaultPropertyRole.Technical);
        loadTimestamp.Metadata.SetAnnotation(DataVaultAnnotationNames.TechnicalColumnRole, TechnicalMetadataColumnRole.LoadTimestamp);

        var recordSource = entityBuilder.IndexerProperty<string>("RecordSource");
        recordSource.Metadata.SetAnnotation(DataVaultAnnotationNames.PropertyRole, DataVaultPropertyRole.Technical);
        recordSource.Metadata.SetAnnotation(DataVaultAnnotationNames.TechnicalColumnRole, TechnicalMetadataColumnRole.RecordSource);
