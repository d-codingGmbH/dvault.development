...
public sealed class PitAsOfReadContractSnapshotTests {
  private const string ApprovedSnapshot = """
      ...
      - Bounded link-parent PITs use the same ParentHashKey contract with the link hash key.
      ...
      Scenario: link-parent PIT row key
        metadata:
          link CustomerOrder participants [Customer, Order]
          satellite State parent CustomerOrder payload [State Code]
          satellite Fulfillment parent CustomerOrder payload [Fulfillment Location]
          pit CustomerOrder/State/Fulfillment declared satellites [State, Fulfillment]
        request:
          parent hash keys [customer-order-link-hash-001]
          AsOf 2026-05-11T12:00:00+00:00
        expected result:
          ParentHashKey is the CustomerOrder link hash key.
          Satellite snapshots resolve from link-parent satellite rows attached to CustomerOrder.
          The public field names remain ParentHashKey, LoadTimestamp, and <Satellite>LoadTimestamp.
      ...
      Diagnostics:
        ...
        unsupported bridge-driven read:
          fail deterministically before row materialization.
        unsupported parent kind other than hub or link:
          fail deterministically before row materialization.
        satellite attached to a different parent or mixed hub/link parent shape:
          fail deterministically before row materialization.
        model-first dvault.model.v1 link-parent PIT artifact:
          remain outside this runtime contract; v1 PIT artifacts are hub-parent-only.
