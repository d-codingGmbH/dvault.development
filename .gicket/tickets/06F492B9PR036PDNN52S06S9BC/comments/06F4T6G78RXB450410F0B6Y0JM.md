...
- Unsupported inferred API claim in contract: Cover :: - Cover the existing explicit and registry-backed read-diagnostics entry points where public APIs already exist, including `DataVaultRegistryLatestSatelliteReadRequest` and `DataVaultRegistryBridgeReadRequest`.
...
- Unsupported inferred API claim: Cover :: - Cover the existing explicit and registry-backed read-diagnostics entry points where public APIs already exist, including `DataVaultRegistryLatestSatelliteReadRequest` and `DataVaultRegistryBridgeReadRequest`.
...
- critic-item-3 [blocking-finding] Unsupported inferred API claim: Cover :: - Cover the existing explicit and registry-backed read-diagnostics entry points where public APIs already exist, including `DataVaultRegistryLatestSatelliteReadRequest` and `DataVaultRegistryBridgeReadRequest`.

Missing examples / edge cases
- none

Risky assumptions
- Existing API/type assumption lacks source evidence: Cover :: - Cover the existing explicit and registry-backed read-diagnostics entry points where public APIs already exist, including `DataVaultRegistryLatestSatelliteReadRequest` and `DataVaultRegistryBridgeReadRequest`.
