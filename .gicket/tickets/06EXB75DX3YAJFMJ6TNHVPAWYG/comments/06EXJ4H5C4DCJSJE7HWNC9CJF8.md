...
- Unsupported inferred API claim in contract: Custom :: - Custom naming-policy coverage must be broad enough to prove override behavior without forcing every property-column normalization detail into the override contract unless the existing API already does so.
...
- Unsupported inferred API claim: Custom :: - Custom naming-policy coverage must be broad enough to prove override behavior without forcing every property-column normalization detail into the override contract unless the existing API already does so.
...
- critic-item-3 [blocking-finding] Unsupported inferred API claim: Custom :: - Custom naming-policy coverage must be broad enough to prove override behavior without forcing every property-column normalization detail into the override contract unless the existing API already does so.

Missing examples / edge cases
- none

Risky assumptions
- Existing API/type assumption lacks source evidence: Custom :: - Custom naming-policy coverage must be broad enough to prove override behavior without forcing every property-column normalization detail into the override contract unless the existing API already does so.
