...
- Unsupported inferred API claim: Replaced, EF :: - Replaced inferred EF API assumptions with source-backed branch evidence: the current branch only exposes `ModelBuilder.UseDataVault()` as a conventions marker plus provider-neutral modeling and metadata types under `src/DCoding.Data.DVault/Modeling`, so this ticket may introduce the explicit EF translation entry point and any minimal supporting public surface; no child tickets, relations, attachments, or planning documents were created.
...
- critic-item-3 [blocking-finding] Unsupported inferred API claim: Replaced, EF :: - Replaced inferred EF API assumptions with source-backed branch evidence: the current branch only exposes `ModelBuilder.UseDataVault()` as a conventions marker plus provider-neutral modeling and metadata types under `src/DCoding.Data.DVault/Modeling`, so this ticket may introduce the explicit EF translation entry point and any minimal supporting public surface; no child tickets, relations, attachments, or planning documents were created.

Missing examples / edge cases
- none

Risky assumptions
- Existing API/type assumption lacks source evidence: Replaced, EF :: - Replaced inferred EF API assumptions with source-backed branch evidence: the current branch only exposes `ModelBuilder.UseDataVault()` as a conventions marker plus provider-neutral modeling and metadata types under `src/DCoding.Data.DVault/Modeling`, so this ticket may introduce the explicit EF translation entry point and any minimal supporting public surface; no child tickets, relations, attachments, or planning documents were created.
