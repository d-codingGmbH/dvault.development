...
- at least 50 total hub, link, and satellite operations;
...
The ticket-specific configured SQL Server benchmark triplet is:
...
- `fallbackCauses=none`;
- mean `139.368` ms for the observed run context.
...
- mean `358.288` ms for the observed run context.

## Historical Threshold Bundle

The historical threshold bundle remains authoritative for the bounded 50/500 gate and fallback vocabulary:
...
That bundle records completed scale rows below, inside, and above the SQL Server native-bulk gate. It keeps the same 50 minimum-operation and 500 maximum-satellite-operation decision, and it corrects fallback-row wording so fallback rows report `executionPath=DVault provider-neutral fallback path`, `selectedStrategy=<none>`, and the SQL Server candidate only as diagnostics.
