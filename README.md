...
```sh
dotnet add package DCoding.Data.DVault --version 0.20.0
dotnet add package DCoding.Data.DVault.Sqlite --version 0.20.0
dotnet add package DCoding.Data.DVault.Postgres --version 0.20.0
dotnet add package DCoding.Data.DVault.MySql --version 0.20.0
dotnet add package DCoding.Data.DVault.Oracle --version 0.20.0
dotnet add package DCoding.Data.DVault.SqlServer --version 0.20.0
dotnet add package DCoding.Data.DVault.Analyzers --version 0.20.0
```
...
PIT-backed reads target one `DataVaultPitMetadata` declaration, explicit parent hash keys, and an `asOf` timestamp. For ordinary PITs, one selected PIT row is returned per requested parent. For the bounded multi-active PIT baseline, all referenced multi-active hub-parent satellites must share the same canonical driving-key names and order; reads keep the parent-hash-key request surface and return one visible row per parent and driving-key tuple. `ReadPitRowsAsync(...)` returns raw `DataVaultPitReadRecord` rows with the PIT driving-key values; `ReadPitAsync(...)` maps selected rows through a caller-owned projection delegate with exact-name access to `ParentHashKey`, the canonical driving-key names, `LoadTimestamp`, and declared satellite segments.
...
## v0.20.0 Provider-Optimized Write Boundary
