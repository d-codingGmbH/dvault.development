...
The current coordinated release baseline is [DVault v0.32.0 Release Notes](docs/releases/v0.32.0.md), which documents benchmark-driven provider threshold evidence, the review-only provider-specific SQL artifact manifest lane, and carried-forward package scope without recording package publication. Earlier release notes remain historical feature-introduction records. For detailed performance guidance, see [Performance Profiles](docs/performance-profiles.md). For a short adopter readiness pass before production use, see the [Production Adoption Checklist](docs/production-adoption-checklist.md).
...
The v0.29.0 provider schema guardrail baseline remains documented in [DVault v0.29.0 Release Notes](docs/releases/v0.29.0.md), is carried forward by the current v0.32.0 documentation baseline, and is anchored by [Provider Identifier And DDL Guardrail Contract](docs/plans/provider-identifier-ddl-guardrail-contract.md). Logical DVault names remain provider-neutral and traceable through `DataVaultAnnotationNames.ProducedName`; supported provider profiles may derive safe physical names only when generated DVault-owned tables, columns, keys, indexes, or constraints would otherwise be unsafe for the selected provider.
...
The v0.32.0 release record is the current coordinated seven-package baseline for benchmark-driven provider threshold evidence and the review-only provider-specific SQL artifact manifest lane. See `docs/releases/v0.32.0.md` for package scope, boundary shift from v0.31.0, benchmark evidence anchors, the design-time `sql-artifact` command surface, validation surfaces, and non-goals.

It carries forward the v0.31.0 performance decision-tree and observability baseline, v0.30.0 typed helper support-bundle freshness baseline, v0.29.0 provider schema guardrail baseline, v0.28.0 provider read optimization evidence boundary, v0.27.0 EF lifecycle analyzer guardrails, v0.26.0 provider-tuning diagnostics and benchmark verifier evidence, v0.25.0 ReadShape and typed helper boundary, v0.24.0 async streaming and EF safety boundary, and v0.21.0 PIT/bridge maintenance boundary from earlier releases.

Notable user-facing changes:

- Provider save threshold guidance now has checked-in v0.32 benchmark evidence bundles for local Podman PostgreSQL, SQL Server, MySQL, and Oracle scale runs under the shared artifact-triplet contract.
...
- The new `sql-artifact` design-time verb emits a `dvault.sql-artifact.v1` SQL Server dry-run manifest for `provider-native-bulk-ingestion` review only; it does not emit deployable SQL payload files, auto-deploy artifacts, or create runtime dispatch.
- No package publication, automatic stored-procedure generation, runtime artifact invocation, migration synchronization, provider support expansion, DBA workflow automation, credential handling, or release automation is introduced by the v0.32.0 release baseline.
...
- At the v0.26.0 release boundary, SQLite remained the only repository-proven optimized latest-satellite, PIT, and bridge read provider path. The current v0.32.0 baseline preserves SQLite-only latest-satellite optimization and documents later PIT/bridge candidate evidence for PostgreSQL, SQL Server, MySQL, and Oracle.
