...
Consumer applications install the provider-neutral package and exactly one provider package for the database they use. Keep every DVault package on one aligned version. Use `8.43.0` for `net8.0` and EF Core 8 projects, or `10.43.0` for `net10.0` and EF Core 10 projects; do not use a consumer-facing `0.43.0` package version.
...
The analyzer package is optional and should usually be referenced with `PrivateAssets="all"` in consumer projects that own DVault Code-First declarations or compile-time generated row mapping declarations. Use `8.43.0` or `10.43.0` to match the runtime and provider package line.
