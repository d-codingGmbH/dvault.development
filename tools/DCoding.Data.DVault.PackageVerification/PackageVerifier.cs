...
public sealed class PackageVerifier {
  ...
  private const string ExpectedAnalyzerBuildHostGuidance = "Build projects that reference `DCoding.Data.DVault.Analyzers` with a `.NET 10 SDK` host, including `net8.0` projects using the `8.40.0` package line.";
  ...
  private static readonly ExpectedPackageLine[] ExpectedPackageLines = [
      new("8.40.0", Net8TargetFramework, "EF Core 8"),
      new("10.40.0", Net10TargetFramework, "EF Core 10"),
  ];
  ...
  private static void ValidateReadmeDoesNotUseDisallowedInstallVersions(
    ...
    foreach (var disallowedFragment in DisallowedInstallVersionFragments) {
      if (archive.ReadmeText?.Contains(disallowedFragment, StringComparison.Ordinal) == true) {
        issues.Add(new PackageVerificationIssue(
            archive.Id,
            "Packaged README.md must not document stale or planning-release install version fragment '" + disallowedFragment + "'; use separate 8.40.0 and 10.40.0 package-line guidance."));
      }
    }
  }
  ...
  private static void ValidateReadmeContainsAnalyzerBuildHostGuidance(
    ...
    if (archive.ReadmeText?.Contains(ExpectedAnalyzerBuildHostGuidance, StringComparison.Ordinal) != true) {
      issues.Add(new PackageVerificationIssue(
          archive.Id,
          "Packaged README.md must state that DCoding.Data.DVault.Analyzers is supported on the .NET 10 SDK build-host baseline, including net8.0 projects using the 8.40.0 package line."));
    }
  }
